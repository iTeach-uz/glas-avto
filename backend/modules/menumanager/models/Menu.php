...
class Menu extends \kartik\tree\models\Tree
{
    ...
    public function fields(){
        return [
            'id',
            'menuName' => function($model){
                return $model->title;
            },
            'to' => function($model){
                return "/".$model->url_value;
            },
            'subMenus'=>function(Menu $model){
                $subMenus=$model->activeSubMenus;
                $subMenu_parse=[];
                 foreach ($subMenus as $key=>$subMenu){
                     $subMenu_parse[$key]['key']=$subMenu->id;
                     $subMenu_parse[$key]['text']=$subMenu->title;
                     $subMenu_parse[$key]['to']="/".$subMenu->url_value;
                 }
                 return $subMenu_parse;
            }
            ];
    }
